...
export function useCaseDetailActions(
  ...
  const saveCaseDetailCache = (data: Omit<CaseDetailCache, 'tenantId'>) => {
    try {
  ...
  const loadCaseDetailCache = (): CaseDetailCache | null => {
    try {
  ...
  const applyCaseDetailCacheIfAny = (isOffline: boolean) => {
  ...
  }, [caseData.id, setSessions, setDocs, profile?.tenant_id]);
